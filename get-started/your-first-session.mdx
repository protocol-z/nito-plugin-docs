...
- **Your machine keeps the record.** History and transcripts go to a dedicated Nito profile directory at `~/.nito/hosts/<host>/<profile>/`, separate from your harness's own configuration. The directory is restricted to your user account, and Nito does not encrypt what is inside it. Deleting it removes the record. See [Uninstall](/get-started/uninstall).
