...
<CardGroup cols={2}>
  ...
  <Card title="Enter full Nito sessions" icon="window">
    `/nito:session` opens a fresh session in your harness where you can access and switch between models in the Nito catalog, with your history retained for future sessions. Enter `/nito:private` with no prompt for incognito sessions: full conversations without prompts or responses being recorded upstream.
  </Card>
  <Card title="Deploy multiple models on one prompt" icon="layer-group">
    `/nito:fusion` sends one prompt to two or three models, then returns a synthesized response.
  </Card>
</CardGroup>
