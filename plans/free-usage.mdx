...
A request is an [`ask`](/commands/ask-and-continue) or an inline [`private`](/commands/private) question that reaches one of the select models available on Free. Each counts once. Full [sessions](/commands/session) are not part of the free allowance.
