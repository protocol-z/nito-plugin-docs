...
description: "A complete terminal workflow that reviews your changes across several Private models with Fusion, so proprietary code never reaches your IDE's provider. Includes a production-ready script, output you can act on, and privacy notes."
---

**Goal.** Get a thorough, multi-model review of your changes without sending proprietary code to your IDE's model provider. This recipe builds a reusable script that reviews a git diff from your terminal, on **Private models** (open-weight models with retention switched off upstream), using **Fusion** so several models review at once and their findings are reconciled into one report.
...
**It uses Private models.** Every Nito call runs at the privacy level of the model it uses. Private models are open-weight models routed with **zero data retention** enforced upstream: the provider is instructed not to store your prompt or the response. So the review goes to a model that is told to forget it, not to a frontier provider that may retain it. See [Privacy Levels](/privacy/levels) for the full ladder.
...
List the Private models your account can use and pick two, ideally with different strengths (a coder-tuned model plus a strong general model):

```bash
# Show only Private models, then note two IDs
nito agent models
```

`qwen/qwen3-coder` (code-tuned) and `z-ai/glm-5.2` (strong general) are a good default pair. Any two distinct Private models work; picking different families surfaces more.
...
# --- Configuration ----------------------------------------------------------
# Private reviewers. Add a third for a wider panel.
...
**Review from inside your chat.** To review with your harness's own model held back rather than from the terminal, use [`private`](/commands/private): `/nito:private` (Claude Code) or `$nito:private` (Codex) runs the turn incognito so your harness never sends the code upstream.
...
- **Private tier is retention-off, not secrecy.** The provider is instructed not to store your code; it still processes it to produce the review. For a stronger guarantee where the provider cannot read the input at all, the [Confidential and Encrypted levels](/privacy/levels) run inside attested hardware, subject to model availability.
- **Fusion mixes levels by the strictest rule.** A Fusion of Private models stays Private. Mixing in a lower level would pull the run down, so keep every reviewer at Private or stronger. See [Fusion](/commands/fusion).
...
| "did not complete" or empty output | A chosen model is momentarily down | Swap in another Private model in the `MODELS` array |
| A privacy-level error from Fusion | A non-Private model was mixed in | Keep every reviewer at Private tier or stronger |
...
<CardGroup cols={2}>
  ...
  <Card title="Privacy Levels" icon="shield-check" href="/privacy/levels">
    What Private tier guarantees, and the stronger tiers above it.
  </Card>
</CardGroup>
