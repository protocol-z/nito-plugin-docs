...
- **Nito installed and signed in** in Claude Code or Codex. See [Install on Claude Code](/get-started/install-claude-code) or [Install on Codex](/get-started/install-codex).
- **A paid plan** (Fusion is a paid feature). The single-model check below works on Free.

## Step 1: Get your harness's own answer
...
Your harness's own model answers. Keep that answer in mind for the comparison.
...
## Step 3: Read the synthesis against your harness's own model
...
Now compare against your harness's own model:

- **The panel agrees with your harness's own model:** you have independent corroboration. Move on.
...
- **The panel disagrees with your harness's own model:** the strongest signal. Read why, and re-examine that answer before you act on it.
...
<Tabs>
  <Tab title="Claude Code">
    ```text
    /nito:ask --model openai/gpt-5.5 Is this correct? "A UUID v4 is guaranteed unique across systems." Answer yes or no and explain in one sentence.
    ```
  </Tab>
  <Tab title="Codex">
    ```text
...
| The panel just echoes your harness's own model | The models chosen are too similar | Pick models from different families (a frontier model plus an open-weight one) |
...
<CardGroup cols={2}>
  ...
  <Card title="Private Code Review Pipeline" icon="shield-halved" href="/cookbook/private-code-review">
    Multi-model review of your changes on Private models.
  </Card>
