...
<Steps>
  <Step title="Install and sign in">
    Install the command line tool (see [Install on Claude Code](/get-started/install-claude-code)), then sign in once. Nito authenticates through `nito agent login` and stores a credential in your OS keychain, so no key or token goes into your host config.
