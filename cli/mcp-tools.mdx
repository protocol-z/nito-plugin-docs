...
<Steps>
  <Step title="Install and sign in">
    Install the command line tool (see [Install on Claude Code](/get-started/install-claude-code)), then sign in once. Unlike servers that take an API key in the config, Nito authenticates through `nito agent login` and stores a credential in your OS keychain, so no key or token goes into your host config.
  ...
  <Step title="Restart your MCP host">
    Your agent now has the Nito tools available. Try asking it to list Nito models or run a completion.
  </Step>
</Steps>
